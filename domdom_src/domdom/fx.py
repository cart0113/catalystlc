...
class ChangeOnMouseMove(Fx):
    ...
    def get_js_create_command(self):
        ...
        id = '"' + self.get_full_id() + '"'        
        bbox        = 'null' if self.bbox        is None else '"' + self.bbox.get_full_id() + '"'
        ...
        e0, e1, e2, e3 = '"' + move_start[0].get_full_id() + '"', '"' + move_start[1] + '"', '"' + move_stop[0].get_full_id() + '"', '"' + move_stop[1] + '"' 
        
        command =  'domdom.events[' + id + '] = new domdom.Fx_OMM(' + id + ',"' + self.parent.get_full_id() + '",'
